 ...
 * @LastEditTime: 2020-08-08 16:16:54
 */
...
$("#loginForm").submit((event) => {
  ...
  if (!getEmail || !getPassword || !regex.test(getEmail)) {
    ...
    if (!$(".missingInput").length) {
      $(".clientsideLoginErrors").append(
        `<p class="missingInput">You must provide a valid email address and password</p>`
      );
    ...
    $.ajax(requestConfig).then(function (responseMessage) {
      if (responseMessage.error) {
        ...
        if (!$(".responseError").length) {
          $(".clientsideLoginErrors").append(
            `<p class="responseError">${responseMessage.error}</p>`
          );
          $(".clientsideLoginErrors").show();
        }
